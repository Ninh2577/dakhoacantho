...
<div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden flex flex-col group hover:shadow-md hover:border-slate-200 transition-all duration-300">
    <!-- Thumbnail Image -->
    <a href="{{ url($article->category->slug . '/' . $article->slug) }}" class="block relative aspect-video overflow-hidden bg-slate-100">
    ...
    <div class="p-5 flex-grow flex flex-col justify-between">
        <div class="space-y-3">
            ...
            <h3 class="text-base font-extrabold text-slate-900 group-hover:text-clinic-blue leading-snug tracking-tight transition-colors line-clamp-2">
                <a href="{{ url($article->category->slug . '/' . $article->slug) }}">
                    {{ $article->title }}
                </a>
            </h3>
        ...
        <div class="pt-4 border-t border-slate-50 mt-4 flex items-center justify-end">
            <a href="{{ url($article->category->slug . '/' . $article->slug) }}" class="inline-flex items-center text-xs font-extrabold text-clinic-teal hover:text-clinic-blue transition-colors">
                <span>Chi tiết</span>
